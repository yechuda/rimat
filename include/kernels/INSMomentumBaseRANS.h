...
/****************************************************************/
#ifndef INSMOMENTUMBASERANS_H
#define INSMOMENTUMBASERANS_H

#include "Kernel.h"

// Forward Declarations
class INSMomentumBaseRANS;

template<>
InputParameters validParams<INSMomentumBaseRANS>();
 ...
 * .) INSMomentumLaplaceFormRANS
 * .) INSMomentumTractionFormRANS
 * depending on the application.  For "open" flow boundary conditions,
 * the INSMomentumLaplaceFormRANS seems to give better results.  If you
 * have traction BCs, i.e. BCs where the normal traction is specified
 * on part of the boundary, you should use the INSMomentumTractionFormRANS
 * instead.
 */
class INSMomentumBaseRANS : public Kernel
{
public:
  INSMomentumBaseRANS(const InputParameters & parameters);

  virtual ~INSMomentumBaseRANS(){}
